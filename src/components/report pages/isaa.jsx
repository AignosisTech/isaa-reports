...
const ISAA = () => {
  ...
  function calculateAge(dob) {
    ...
    let month = parseInt(dob.substring(4, 6), 10) - 1; // Month is 0-based in JS Date
    let day = parseInt(dob.substring(6, 8), 10);

    // Create birthdate and current date objects
  ...
  useEffect(() => {
    ...
    if (age != undefined)
      setAgeString(`${age.years} years ${age.months} months`);
  }, [ageString]);
        ...
        <div className="pdf-page bg-white p-8 shadow-md rounded-md w-[210mm] h-[297mm]">
          ...
          >
            ...
            <div style={{ textAlign: "justify" }}>
              ...
              interaction with subjects and supplemented by parents or
              caretakers in order to diagnose autism. ISAA consists of 40 items
              rated on a 5 scale ranging from 1 (never) to 5 (always). The 40
              items on ISAA are further divided into the following sub scales.
              <br />
          ...
          <div><span className="label">Age:</span> <span id="chronologicalAgecell"></span></div>
            ...
            <center>
              <div style={{ fontSize: 20, fontWeight: 650 }}>
                ...
                <br></br>
                <span>Age: </span>
                <span style={{ color: "#94059f" }}>{ageString}</span>
              </div>
              <br></br>
              <table
                border="4"
...
domain.</p> */}
            The Scores on ISAA were indicative of ({patientData.TOT_ISAA}){" "}
            {patientData.isaaInterpretation} degree of autistic traits being
            present in the child.
            <br />
            {patientData.TOT_ISAA >= 63 && patientData.TOT_ISAA <= 69 && (
              <p>
                Scores ({patientData.TOT_ISAA}) on ISAA suggest the presence of
                some autistic traits, although they do not meet the threshold
                for a conclusive diagnosis of autism spectrum disorder.
              </p>
            )}{" "}
            <br />
            <br />
